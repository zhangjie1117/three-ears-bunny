<template>
  <div class="container">
    <div v-if="isSoldOut == false">
      <img class="rabbit" src="../assets/images/bunny.png" alt="" />
      <img class="jar" src="../assets/images/jar1.png" alt="" />
      <div class="content">
            ...
            >
              <template v-if="enableMint">
                ...
                <div class="eth">
                  ...
                  <span>{{
                    currentPhase <= 1
                      ? Math.round(guaranteeWhiteListPrice * count * 1000) /
                        1000
                      : currentPhase <= 2
                      ? Math.round(nonGuaranteeWhiteListPrice * count * 1000) /
                        1000
              ...
              <div class="mint" v-if="loading == 'wait'">
                ...
                <div class="time time1-i">
                  <span>{{ countDownTime[0] }}</span>:<span>{{ countDownTime[1] }}</span
                  >:<span>{{ countDownTime[2] }}</span>
                </div>
              </div>
            </div>
            <div
              @click="goOpensea"
              v-if="loading == 'close'"
              :class="['ml-95','opensea',loading == 'close'?'fixedPos1':'']"
            >
              <img src="../assets/images/opensea.png" alt="" />
              Mint Successful
            </div>
            <div
              :class="['ml-95', 'opensea', loading == 'on' ? 'fixedPos' : '']"
              v-if="loading == 'on'"
            >
              Minting
              <div class="dots">
...
export default {
  ...
  computed: {
    ...
    formatTime() {
      return function (timestamp) {
        ...
        var minutes = new Date(timestamp)
          .getMinutes()
          .toString()
          .padStart(2, "0");
        var en_mon_arr = [
  ...
  methods: {
    ...
    getTimeZone() {
      ...
      const sign =
        timeZoneOffsetHours > 0 ? "-" : timeZoneOffsetHours == 0 ? "" : "+";
      const formattedOffset = `UTC${sign}${
        Math.abs(timeZoneOffsetHours).toString() == 0
          ? ""
    ...
    countDown() {
      ...
      } else if (now > this.non_guarantee_white_list_end_time) {
        this.currentPhase = 3;
